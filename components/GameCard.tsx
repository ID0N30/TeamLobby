...
const GameCard: React.FC<GameCardProps> = ({ game, currentUserId, onVote, onOpenDetails, isVotingEnabled }) => {
  ...
  return (
    <div 
      onClick={() => { soundService.playPop(); onOpenDetails(game); }}
      ...
      <div className="relative h-44 w-full overflow-hidden shrink-0 bg-gray-900 flex items-center justify-center">
        {game.imageUrl && !imgError ? (
            ...
            <div className="flex flex-col items-center justify-center text-gray-700">
                ...
                <span className="text-[8px] uppercase font-black tracking-widest">{imgError ? t('lobby.linkError') : t('lobby.noCover')}</span>
            </div>
        )}
        ...
        <div className="absolute bottom-3 left-4 right-4">
          ...
          <p className="text-[10px] text-primary font-black uppercase tracking-widest mt-1 drop-shadow-md">{t(`genre.${game.genre}` as TranslationKey) || game.genre}</p>
        </div>
      </div>
